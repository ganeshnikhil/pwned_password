...
def lookup_pwned_api(pwd):
    ...
    """
    # convert you password into a sha1 hash 
    sha1pwd = hashlib.sha1(pwd.encode('utf-8')).hexdigest().upper()
    
    # split the password in two parts one part in head and rest in tail
    head, tail = sha1pwd[:5], sha1pwd[5:]
    
    # pwnedpasswords api to send request 
    url = 'https://api.pwnedpasswords.com/range/' + head
    
    # using get request to do that
    ...
    if not res.ok:
        raise RuntimeError('Error fetching "{}": {}'.format(
            url, res.status_code))
    # spit the password and convert it into multipart sublists 
    # hashed passwrod and no time it leaked  
    hashes = (line.split(':') for line in res.text.splitlines())
    # iterate over each  and check if your passwrod tail matches with the request hash 
    # if fount count = no of time it (pwned/leaked)
    # else count = 0 
